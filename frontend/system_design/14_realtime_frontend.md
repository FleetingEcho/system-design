...
**踩坑1：WebSocket 断线后未实现自动重连**
❌ 错误：WebSocket 断开后（网络抖动/服务重启）不重连，用户需要刷新页面才能恢复实时更新，体验极差。
✓ 正确：监听 `onclose` 事件，用指数退避（1s → 2s → 4s → max 30s）+ 抖动自动重连，重连时重新订阅频道。
原因：WebSocket 连接在真实网络环境中不稳定，自动重连是生产级实时应用的必备能力。

**踩坑2：多 Tab 建立多个 WebSocket 连接浪费资源**
❌ 错误：用户打开 5 个 Tab，每个 Tab 建立一个 WebSocket 连接，服务端承受 5 倍连接压力。
✓ 正确：用 SharedWorker 维护一个共享 WebSocket 连接，所有 Tab 通过 SharedWorker 收发消息。
原因：WebSocket 是长连接，每个 Tab 独立连接在用户多开时成倍消耗服务端连接资源。

**踩坑3：乐观更新后服务端确认失败不回滚**
❌ 错误：发送聊天消息时立即显示（乐观更新），但 WebSocket 发送失败后不回滚，消息"假发送"永久留在 UI。
✓ 正确：乐观更新时记录 `pendingId`，收到服务端确认后用真实 ID 替换；超时或失败时移除消息并显示错误提示。
原因：乐观更新必须配套失败回滚机制，否则 UI 状态与服务端数据不一致。

**踩坑4：频繁的小数据更新未做节流/批量合并**
❌ 错误：每次光标移动都通过 WebSocket 广播坐标（60fps），导致消息洪流，接收方渲染跟不上，UI 抖动严重。
✓ 正确：高频事件用 throttle（如 50ms 发一次光标位置）；服务端批量推送时客户端做补间动画而非逐帧更新。
原因：WebSocket 不是 UDP，每条消息都有协议开销，高频小消息比低频大消息更低效。
