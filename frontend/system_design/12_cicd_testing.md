...
**踩坑1：测试实现细节而非行为**
❌ 错误：测试中直接调用组件的内部方法或断言内部 state，组件重构后测试全部失败，即使行为完全相同。
✓ 正确：通过公开的用户交互（点击、输入）和可见的输出（DOM 内容、网络请求）测试组件行为，不 mock 内部实现。
原因：测试应该在重构后保持绿色（除非行为改变），测试内部实现会与代码结构深度耦合，阻碍重构。

**踩坑2：E2E 测试覆盖范围过广导致套件极慢**
❌ 错误：用 Playwright 测试所有业务流程，E2E 套件运行 2 小时，PR 等待时间不可接受，开发者开始绕过 CI。
✓ 正确：E2E 只覆盖最关键的"Happy Path"（注册、登录、下单），边界情况和错误处理用单元/集成测试覆盖。
原因：测试金字塔原则——E2E 贵且慢，应该是少而精；大量场景用快速的单元测试覆盖。

**踩坑3：视觉回归测试产生大量误报（Flaky Tests）**
❌ 错误：截图比对测试在不同机器/系统上像素级差异（字体渲染、抗锯齿），导致 CI 上频繁失败但本地没问题。
✓ 正确：用 Chromatic 等云端视觉测试服务，固定渲染环境；或设置合理的像素差异容差；所有截图在相同 Docker 环境中生成。
原因：视觉测试的像素比对对环境高度敏感，跨平台或字体版本差异都会触发 false positive。

**踩坑4：部署流水线未做金丝雀发布，问题影响全量用户**
❌ 错误：每次发布直接全量替换，新版本有严重 bug 时已影响所有用户，需要紧急回滚（MTTR 高）。
✓ 正确：流量逐步放量（1% → 5% → 25% → 100%），每个阶段监控错误率和核心指标，异常时自动回滚。
原因：金丝雀发布将爆炸半径（Blast Radius）限制在小比例用户，大幅降低发布风险。
