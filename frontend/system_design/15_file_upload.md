...
**踩坑1：大文件一次性读入内存导致浏览器崩溃**
❌ 错误：`const buffer = await file.arrayBuffer()`，将 2GB 视频全部读入内存，浏览器 Tab 崩溃或内存不足报错。
✓ 正确：用 `file.slice(start, end)` 分块，每次只读取一个 chunk（如 5MB）到内存，上传完毕后 GC 释放，下一块再读。
原因：`file.slice()` 返回 Blob 引用，不复制数据，只有 `.arrayBuffer()` 时才真正分配内存，分块上传峰值内存 = 并发数 × 块大小。

**踩坑2：直传 S3 未设置 CORS 和预签名 URL 过期时间**
❌ 错误：预签名 URL 有效期设为 7 天（担心上传慢），但攻击者拿到 URL 后可以在 7 天内任意上传任意内容到 S3 Bucket。
✓ 正确：URL 有效期设为上传所需时间的 1.5 倍（如 15 分钟），结合 S3 CORS 配置只允许指定域名上传，禁止其他来源。
原因：预签名 URL 是临时访问凭证，过期时间越长暴露窗口越大，应尽量短并配合 CORS 限制来源。

**踩坑3：文件 Hash 在主线程计算阻塞 UI**
❌ 错误：用 `crypto.subtle.digest('SHA-256', file.arrayBuffer())` 在主线程计算 1GB 文件 Hash，页面冻结 5-10 秒。
✓ 正确：将 Hash 计算放入 Web Worker（`postMessage(fileSlice)`），主线程不阻塞，用进度条展示 Hash 计算进度。
原因：大文件 Hash 是 CPU 密集型操作，必须在 Worker 线程执行，避免阻塞主线程的渲染和交互。

**踩坑4：并发上传数量不加限制打满带宽或服务端连接**
❌ 错误：文件切成 100 个 chunk 后用 `Promise.all` 同时上传 100 个请求，移动端带宽耗尽，其他网络请求超时。
✓ 正确：用信号量（Semaphore）或 p-limit 限制并发数（如 3-5 个），优先上传失败的 chunk，带宽不足时动态降并发。
原因：浏览器对同域请求有并发限制（HTTP/1.1 下 6 个），超出排队反而更慢；HTTP/2 下无限并发会消耗服务端连接资源。
