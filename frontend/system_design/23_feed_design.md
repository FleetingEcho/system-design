...
**踩坑1：无限滚动时直接追加 DOM 节点不做虚拟化**
❌ 错误：每次加载更多都向 DOM 追加新帖子节点，用户滚动到第 100 页时 DOM 中有 2000+ 节点，滚动帧率下降到个位数。
✓ 正确：无限滚动必须配合虚拟列表，只渲染可视区内的 ~20 个节点，加载更多只是扩展数据源，不增加 DOM 节点数。
原因：DOM 节点数量与渲染成本成正比，大量节点的 Layout 和 Paint 在每次滚动时都要重新计算。

**踩坑2：乐观更新点赞后服务端返回失败不回滚**
❌ 错误：点赞时立即将计数 +1 并改变按钮样式，但请求失败后 UI 没有回滚，用户以为点赞成功实际未生效。
✓ 正确：乐观更新前记录原始状态，请求失败时回滚到原始值并展示错误提示；或用 TanStack Query 的 `onMutate`/`onError` 钩子自动处理。
原因：乐观更新是"先假设成功"，必须有配套的失败回滚机制，否则 UI 与服务端数据不一致。

**踩坑3：用轮询而非 WebSocket 实时推送新帖**
❌ 错误：每 5 秒 fetch 一次新帖，对于大量在线用户来说服务端压力极大，且 5s 延迟对实时 Feed 体验差。
✓ 正确：用 WebSocket 或 Server-Sent Events 推送新帖通知，新帖只更新"新内容"计数 badge，用户点击后才加载。
原因：轮询的服务端请求量 = 用户数 × 轮询频率，百万用户 5s 轮询 = 200k rps，不可扩展。

**踩坑4：Feed 滚动后返回丢失滚动位置**
❌ 错误：用户滚到第 50 条帖子后点击进入详情，返回时回到顶部，需要重新滚动，体验差。
✓ 正确：保存并恢复滚动位置——离开 Feed 时将 `scrollTop` 和 `itemRange` 存入 sessionStorage 或路由状态，返回时恢复。
原因：虚拟列表在组件卸载后 DOM 节点销毁，浏览器原生的滚动位置恢复失效，需要手动管理。
