...
**踩坑1：A/B 测试分组不稳定（同一用户每次刷新看到不同版本）**
❌ 错误：用 `Math.random() < 0.5` 在客户端分组，每次页面加载重新随机，同一用户体验不一致，统计数据混乱。
✓ 正确：用用户 ID 或设备 ID 做确定性哈希分组（`hash(userId + experimentId) % 100 < 50`），同一用户始终在同一组。
原因：A/B 测试的统计有效性要求同一用户在整个实验期间看到相同变体，随机分组会引入噪音。

**踩坑2：Flag 堆积成技术债务（Dead Flag Problem）**
❌ 错误：功能上线后 Flag 不删除，代码中遗留大量 `if (flag.newFeature)` 分支，6 个月后没人知道哪些 Flag 还活着。
✓ 正确：每个 Flag 创建时设置 TTL 和 Owner，CI 中检测超期 Flag 自动告警，功能稳定后用 codemod 删除旧分支。
原因：未清理的 Flag 增加代码理解成本，且两套代码路径都需要维护，是典型的增量技术债。

**踩坑3：服务端渲染时 Flag 与客户端不一致导致 Hydration 闪烁**
❌ 错误：SSR 时未注入用户的 Flag 状态，服务端渲染旧版本 HTML，客户端激活后 Flag 判断为新版本，UI 闪烁（FOUC）。
✓ 正确：SSR 时将用户的 Flag 状态序列化注入 HTML（`window.__FLAGS__ = {...}`），客户端直接使用，避免水合差异。
原因：服务端和客户端的 Flag 状态必须一致，否则 Hydration 时 React 发现 DOM 不匹配会重新渲染。

**踩坑4：多个互斥实验的用户分组重叠**
❌ 错误：同时运行"新结账流程"和"新首页布局"两个实验，部分用户同时进入两个实验组，无法分析单一实验的效果。
✓ 正确：互斥实验用流量切片（traffic bucketing）：总流量分为 N 个桶，每个实验占用不重叠的桶区间。
原因：实验间的用户重叠导致混淆效应（Confounding），无法判断哪个改动影响了指标。
